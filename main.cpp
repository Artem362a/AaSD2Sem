...
public:
    struct Edge {
        ...
        Edge() : from(), to(), distance() {}  // Конструктор по умолчанию
...
public:
    ...
    bool is_connected() const {
        ...
        // Обход в глубину для исходного графа
        set<Vertex> visited;
        stack<Vertex> stack;
        ...
        for (auto it = _edges.begin(); it != _edges.end(); ++it) {
            ...
            transposed.add_edge(edge.to, edge.from, edge.distance);  // Инвертируем ребра
        }
    ...
    vector<Edge> shortest_path(const Vertex& from, const Vertex& to) const {
        ...
        // Хранит расстояния от начальной вершины
        map<Vertex, Distance> distances;
        // Хранит предшественников для восстановления пути
        ...
        while (!pq.empty()) {
            ...
            // Получаем все исходящие ребра из вершины u
    ...
    // Вспомогательная функция для вычисления среднего расстояния (с использованием Дейкстры)
    ...
    // Функция для нахождения травмпункта с максимальной средней длиной ребер
    Vertex find_most_remote_clinic() const {
        if (_vertices.empty()) {
            throw runtime_error("Граф пуст");
        }
...
void analyze_samara_clinics() {
    ...
    samaraGraph.add_vertex("Травмпункт №1 (Советской Армии, 56)");
    samaraGraph.add_vertex("Травмпункт №2 (Ново-Садовая, 159)");
    samaraGraph.add_vertex("Травмпункт №3 (Авроры, 150)");
    samaraGraph.add_vertex("Травмпункт №4 (Стара-Загора, 142)");
    samaraGraph.add_vertex("Травмпункт №5 (Металлургическая, 71)");
    samaraGraph.add_vertex("Травмпункт №6 (Победы, 96А)");
    samaraGraph.add_vertex("Травмпункт №7 (Гагарина, 24)");


    samaraGraph.add_undirected_edge("Травмпункт №1 (Советской Армии, 56)",
                       "Травмпункт №7 (Гагарина, 24)", 1.5);
    samaraGraph.add_undirected_edge("Травмпункт №2 (Ново-Садовая, 159)",
                       "Травмпункт №4 (Стара-Загора, 142)", 3.2);
    samaraGraph.add_undirected_edge("Травмпункт №3 (Авроры, 150)",
                       "Травмпункт №6 (Победы, 96А)", 4.1);
    samaraGraph.add_undirected_edge("Травмпункт №4 (Стара-Загора, 142)",
                       "Травмпункт №5 (Металлургическая, 71)", 2.8);
    samaraGraph.add_undirected_edge("Травмпункт №5 (Металлургическая, 71)",
                       "Травмпункт №7 (Гагарина, 24)", 2.3);
    samaraGraph.add_undirected_edge("Травмпункт №6 (Победы, 96А)",
                       "Травмпункт №2 (Ново-Садовая, 159)", 5.0);
    samaraGraph.add_undirected_edge("Травмпункт №1 (Советской Армии, 56)",
                       "Травмпункт №4 (Стара-Загора, 142)", 3.5);
    ...
    for (const auto& clinic : samaraGraph.vertices()) {
        cout << clinic << ": " 
             << samaraGraph.average_edge_length(clinic) << " км\n";
    }
}
